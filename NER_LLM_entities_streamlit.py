...
"""
    ...
    def _try_contextual_geocoding(self, entity, context_clues):
        """Try geocoding with geographical context - dynamic approach.""" 
        if not context_clues:
            return False
        
        # Create context-aware search terms dynamically
        search_variations = [entity['text']]
        
        # Add context to search terms - no hardcoded mappings
        for context in context_clues:
            # Simple approach: just append the context
            search_variations.append(f"{entity['text']}, {context}")
            
            # If context looks like a country code, try full country names
            if context.lower() in ['uk', 'us', 'usa']:
                if context.lower() == 'uk':
                    search_variations.extend([
                        f"{entity['text']}, United Kingdom",
                        f"{entity['text']}, England",
                        f"{entity['text']}, Britain"
                    ])
                elif context.lower() in ['us', 'usa']:
                    search_variations.extend([
        ...
        for search_term in search_variations[:5]:  # Try top 5 with OSM
            try:
                url = "https://nominatim.openstreetmap.org/search"
                params = {
                    'q': search_term,
